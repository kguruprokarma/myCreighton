...
export class Feedback extends React.PureComponent {
  ...
  postFeedback() {
    ...
    reqObj.subject = `${CommonConstants.MY_CREIGHTON} ${reqObj.subject}`;
    reqObj.message = `${reqObj.message} ${this.props.previousURL ? this.props.previousURL : window.location.href}`;
                ...
                <Col xs={8} className='mb5'>
                  <div className='styled-select form-group'>
                    <Field name='subject' component='select' className='form-control input-lg dropDown' onChange={this.selectSubject} value={this.state.subject}>
                      <option disabled value=''>{translateText('common:FEEDBACK_DEFAULT_TOPIC')}</option>
...
const mapStateToProps = (feedbackState) => (
  {
    ...
    previousURL: feedbackState.headerReducer.previousURL
  });
