...
const BusinessInformation = (businessInfoProps) => (
  <article role='article' className='profileRow'>
    ...
    <BasicRow displayName={translateText('common:STAFF_ORGANIZATION')} displayValue={businessInfoProps.data.organization && businessInfoProps.data.organization.split(' ')[0]} />
    <BasicRow
      displayName={translateText('common:STAFF_DEPARTMENT')} displayValue={businessInfoProps.data.organization && businessInfoProps.data.organization.split(' ').map((data, index) => (
