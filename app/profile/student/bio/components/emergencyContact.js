...
const EmergencyContact = (emergencyProps) => (
  <article role='article' className='profileRow mt30'>
    ...
    <Row className='show-grid pt5 pb5'>
      ...
      <Col xs={8} md={9} sm={8} className='dataField'><p className='phoneNumber'><a href={emergencyProps.data.emergency_contact_phone ? `tel:${telephoneCheck(emergencyProps.data.emergency_contact_phone)}` : ''}>{emergencyProps.data.emergency_contact_phone ? telephoneCheck(emergencyProps.data.emergency_contact_phone, '.') : ''}</a></p></Col>
    </Row>
