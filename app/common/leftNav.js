...
const LeftNav = (leftNavProps) => (
  <nav role='navigation' id='localmenu' className='userLeftmenu'>
    ...
    <div className='list-group openSansLight'>
      <Link to={(leftNavProps.role === CommonConstants.STUDENT_LABEL) ? ROUTE_URL.PROFILE : (leftNavProps.role === CommonConstants.STAFF_LABEL ? ROUTE_URL.STAFF : ROUTE_URL.FACULTY + ROUTE_URL.PROFILE)} className='list-group-item' activeClassName='active' >
        {translateText('common:PROFILE_MY_PROFILE')}
      </Link>
      <Link to={(leftNavProps.role === CommonConstants.STUDENT_LABEL) ? ROUTE_URL.ACADEMIC : (leftNavProps.role === CommonConstants.STAFF_LABEL ? ROUTE_URL.PROFESSIONAL : ROUTE_URL.FACULTY + ROUTE_URL.ACADEMIC)} className='list-group-item' activeClassName='active' >
