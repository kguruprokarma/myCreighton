...
import { ConvertTo24Format, ScheduleDays, ConvertDateFromTimeStamp } from '../../../common/utility';
...
const Classes = (classesProps) => (
  <section className='classesWeek' id='classesWeekList'>
    <div className='hidden-xs desktopClassesList'>
      <Link to={`${ROUTE_URL.CLASS_DETAILS}/${classesProps.data.type}/${classesProps.data.sis_source_id}`}>
          <Col md={6} sm={6}>
            <h3 className='pull-left mb0 classesHeading'>{classesProps.data.course_title} {classesProps.data.course_number} </h3>
            <span className='pull-left classOn'>{''}</span>
          </Col>
          <Col md={2} sm={2}>
          ...
          <Col md={2} sm={2} className='text-right'>
            <div className='classTime'>{ScheduleDays(classesProps.data.class_schedule)} {ConvertTo24Format(classesProps.data.class_begin_time)} - {ConvertTo24Format(classesProps.data.class_end_time)}</div>
          </Col>
      </Link>
    </div>
    <Link to={`${ROUTE_URL.CLASS_DETAILS}/${classesProps.data.type}/${classesProps.data.sis_source_id}`}>
    <Row className='visible-xs mobileClassesList row'>
        ...
        <Col xs={5} className='text-right'>
          <span className='classTime'>{ScheduleDays(classesProps.data.class_schedule)} {ConvertTo24Format(classesProps.data.class_begin_time)} - {ConvertTo24Format(classesProps.data.class_end_time)}</span>
        </Col>
      </Row>
    </Link>
  </section>
