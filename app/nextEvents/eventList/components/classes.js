  ...
  <article role='article' className='classesWeek'>
    <div className='hidden-xs desktopClassesList'>
      ...
      <Link to={`${ROUTE_URL.EVENT_DETAILS}/${classesProps.data.type}/${classesProps.data.sis_source_id}/${classesProps.currentIndex}`}>
        <Col md={6} sm={6}>
          <h2 className='mb0 classesHeading'>{(classesProps.data!==null && classesProps.data.course_title)?classesProps.data.course_title:''} {(classesProps.data!==null && classesProps.data.course_number)?classesProps.data.course_number:''} <span className='classOn'>{classesProps.data &&classesProps.data.timeStamp?getDueTime(classesProps.data.timeStamp):''}</span></h2>          
        </Col>
        <Col md={2} sm={2}>
