/*
*Created Date:- 10-02-2017
*Usage:- Re-usable routing url's and parameters
*/

/*
* Relative path url's for components based rendering
*/
export const PROFILE = '/profile'; 
export const ACADEMIC = '/academic';
export const CLASSES = '/classes';
export const CLASS_DETAILS = '/classdetails';
export const DASHBOARD = '/dashboard';
export const TODAY = '/today';
export const WEEK = '/week';
export const LIST = '/list';
export const STUDENT = '/student';
export const STAFF = '/staff';
export const FACULTY = '/faculty';
export const NEXT_EVENTS_STUDENT = '/nextevents'

/*
* Relative path url's parameters for components
*/

export const CATEGORY_NAME_PARAM = '/:categoryname';
export const CLASS_TAB_PARAM = '/:classtab';
export const USER_ROLE_TYPE = '/:roletype';
export const ID_PARAM = '/:id';
export const INDEX_PARAM = '/:index';




