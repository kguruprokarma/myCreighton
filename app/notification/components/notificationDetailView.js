...
const NotificationDetailView = (notificationDetailViewProps) => (
  <section role='region'>
    <h1 className='announced-only'>{translateText('common:NOTIFICATIONS_DETAIL')}</h1>
    <Row className='pb20'>
      <Col md={1} sm={1} xs={1} className='text-center'>
        <button className='btn btn-link btnnoPadding notification-delete' onClick={() => notificationDetailViewProps.deleteNotification(notificationDetailViewProps.detailData)} > <span className='glyphicon glyphicon-remove-sign' aria-hidden='true' /></button>
      </Col>
      <Col md={10} sm={9} xs={8}>
        <p className='notification-title fs1pt06 openSansRegulr notificationHeading pb5'>{notificationDetailViewProps.detailData.title}</p>
