...
export class ClassDetails extends React.PureComponent {
  ...
  render() {
    ...
    let assignmentsData = [];
    let testOrQuizzesData = [];
    let assignmentDue =[];
    let todaysClass = [];
    let upcomingAssignmentsData = [];
    let currentDate = new Date();
    {
        assignments && assignments.length > 0 && assignments.map(function (assignmentObj, i) {
          if (assignmentObj.submission_types === "online_quiz") {
            testOrQuizzesData.push(assignmentObj);
          }else{
            if( assignmentObj.assign_due ){
              let dateValue = DATESCOMPARE(currentDate,assignmentObj.assign_due);;
              if(dateValue === 1) {
      ...
      let index = _.findIndex(obj, { sis_source_id: this.props.params.id });
    ...
    return (
        <section className='classesDeatils'>
          ...
          <PreviousNext presentCategory={this.props.params.categoryname} totalLength={obj.length-1} currentIndex={index} prevItem={prevObject.sis_source_id} nextItem={nextObject.sis_source_id} />
        </section>
    );
  }
}
