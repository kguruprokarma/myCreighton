...
const ClassInfo = (classInfoProps) => (
  <article role='article' className='classesWeek'>
    ...
    <Row className='visible-xs mobileClassesList'>
      ...
      <Col xs={5} className='text-right'>
        <span className='classTime'>{scheduleDays(classInfoProps.data.class_schedule)} {convertTo24Format(classInfoProps.data.class_begin_time)} - {convertTo24Format(classInfoProps.data.class_end_time)}<span> {CommonConstants.TIMEZONE_CT}</span></span>
      </Col>
    </Row>
  </article>
);
