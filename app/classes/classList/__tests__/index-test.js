...
describe('index component testing ----->', () => {
  ...
  const changedProps = {
    getClassesData: () => { },
    getAssignmentDetails: () => { },
    selected: CommonConstants.TODAY,
    onChangeOfTab: () => { },
    onCatagoryChange: () => { },
    convertEncodeURIComponent: () => { },
    onReceiveError: () => { },
    sendLog: () => { },
    params: {
      classTab: CommonConstants.TODAY,
      classtab: CommonConstants.TODAY
    },
    assignmentsData: assignmentData,
    classListData: {
  ...
  const ClassesC1 = shallow(<Classes {...changedProps} />);

  it('WeekDayHeader is defined', () => {
    expect(ClassesC).toBeDefined();
    expect(ClassesC1).toBeDefined();
  });
